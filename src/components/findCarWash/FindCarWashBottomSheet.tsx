...
function getDistanceKm(lat1: number, lng1: number, lat2: number, lng2: number) {
  ...
  const a =
    ...
    Math.sin(dLng / 2) ** 2; // Haversine-formel: beregner luftlinjsafstand i km mellem de to koordinater
  return R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a)); // Returner afstand i km
}

export default function FindCarWashBottomSheet({
  locations,
  selectedLocationPk,
  favoriteIds,
}: FindCarWashBottomSheetProps) {
  ...
  const selectedFacilities = useLocationFilterStore(
  ...
  const washHallNumbers = getUniqueNumbers(
    locations.map((location) => location.car_wash_hall_number),
  );

  const selfWashNumbers = getUniqueNumbers(
    locations.map((location) => location.car_wash_self),
  );
  ...
  const hasFilters =
    ...
    washHallRange.min !== 1 ||
    washHallRange.max !== maxWashHallNumber ||
    selfWashRange.min !== minSelfWashNumber ||
    selfWashRange.max !== maxSelfWashNumber ||
    searchTerm.trim() !== "";
  ...
  const filteredLocations = locations.filter((location) => {
    ...
    const matchesSearch =
      city.includes(searchValue) || address.includes(searchValue);

    const matchesWashHallRange = matchesRange(
      location.car_wash_hall_number,
      washHallRange,
    );

    const matchesSelfWashRange = matchesRange(
      location.car_wash_self,
      selfWashRange,
    );
    ...
    return (
      matchesSearch &&
      matchesWashHallRange &&
      matchesSelfWashRange &&
      matchesFacilities
    );
  });
  ...
  const sortedLocations = [...filteredLocations].sort((a, b) => {
    ...
    return sortDirection === "asc"
      ? waitValueA - waitValueB
      : waitValueB - waitValueA;
  });
  ...
  return (
    <section
      className={`fixed bottom-0 left-1/2 z-50 flex w-full max-w-107 -translate-x-1/2 flex-col rounded-t-3xl bg-background px-4 pt-3 pb-4 text-foreground shadow-2xl ${isDragging ? "" : "transition-all duration-300"}`}
    ...
    >
      <div
        ...
        className="mx-auto mb-3 flex h-7 w-full shrink-0 touch-none cursor-grab select-none items-center justify-center active:cursor-grabbing"
        role="button"
        aria-label="Træk bottom sheet op eller ned"
      >
      ...
      <div className="mb-4 flex w-full shrink-0 flex-col gap-3">
        ...
        <div className="flex items-center gap-6 mt-6 mx-4">
          ...
          <FilterWrapper
            maxWashHallNumber={maxWashHallNumber}
            washHallNumbers={washHallNumbers}
            minSelfWashNumber={minSelfWashNumber}
            maxSelfWashNumber={maxSelfWashNumber}
            selfWashNumbers={selfWashNumbers}
          />
            ...
            return (
              ...
              >
                <CarWashCard
                  ...
                  waitStatus={getWaitStatusForLocation(location)}
                />
              </div>
            );
          })
        ) : (
          <p className="pt-4 text-sm text-(--gray-10)">
            Ingen vaskehaller matcher dine filtre.
          </p>
        )}
      </div>
    </section>
