...
const items: FAQItem[] = [
  ...
  },
  {
    ...
    content: (
      <>
        Med enkeltvask kan du benytte alle vaskehaller uden ekstra betaling. Med
        abonnement vælger du en primær lokation – hvis du vil vaske på andre
        lokationer koster det 10 kr. pr. vask.
        <br /> <br />
        Vil du have adgang til alle vores vaskehaller i landet med dit
        abonnement, kan du opgradere med en tillægspris på 10 kr. pr. måned!
      </>
    ),
  },
];
...
export default function FAQ() {
  ...
  return (
    <section className="bg-(--gray-60) p-6 mt-8 mb-8 rounded-md">
      <h2 className="extra-bold mb-2">FAQ</h2>
      ...
      <div className="space-y-3 max-w-lg w-full">
        ...
        {items.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className={`rounded-md overflow-hidden transition ${isOpen ? "bg-(--gray-10) text-(--foreground-reverse)" : "bg-(--gray-80)"}`}
            >
              {/* TITLE */}
              <button
                onClick={() => setOpenIndex(isOpen ? null : index)}
                className="flex items-center justify-between p-4 text-left w-full hover:cursor-pointer"
              >
              ...
              {/* CONTENT */}
              <div
                className={`px-4 pb-4 transition-all duration-300 light ${isOpen ? "max-h-auto opacity-100" : "max-h-0 opacity-0 overflow-hidden"}`}
              >
                {item.content}
              </div>
            </div>
