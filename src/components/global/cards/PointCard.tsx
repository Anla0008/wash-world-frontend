...
const PointCard = ({ points }: PointProps) => {
    const pointsData = [
        { rabat: "Gratis", type: "Vask selv", antal: 15, fyldetekst: null, href: "/favoritter" },
        { rabat: "-20%", type: "Premium Vask", antal: 200, fyldetekst: "på næste", href: "/favoritter" },
        { rabat: "Gratis", type: "Guldabonnement i en måned", antal: 1500, fyldetekst: null, href: "/favoritter" },
    ];
        ...
        <section className="max-w-lg w-full flex gap-4 overflow-x-auto scroll-smooth hide-scrollbar pt-8 pb-8">
            {pointsData.map((point, index) => {
                ...
                const cardClassName = `relative shrink-0 w-52 px-4 pt-2 rounded-md flex flex-col gap-2 shadow-md  ${
                    isAccessible ? "bg-(--brand-green)" : "bg-foreground"
                }`;
                    ...
                    <>
                        ...
                        <div className="flex flex-col items-center">
                            <div className="flex items-baseline gap-2">
                                <h3 className={`extra-bold text-(--brand-green) uppercase ${isAccessible ? "text-foreground" : "text-(brand-green)"}`}>{point.rabat}</h3>
                                {point.fyldetekst && <p className="text-background">{point.fyldetekst}</p>}
                            </div>
                            <h4 className="text-background extra-bold text-center min-h-12">{point.type}!</h4>
                        </div>
                        <div className="flex justify-between items-baseline">
                            {isAccessible ?(
                                <p className="extra-bold underline bg-(--brand-green-reverse) shadow-lg px-2 rounded-full text-(--gray-80)">Indløs nu!</p>
                            ): <div />} 
