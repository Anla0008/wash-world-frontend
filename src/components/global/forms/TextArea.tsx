  ...
  return (
    <div className="relative w-screen px-15 mt-10">
      ...
      <div className="flex justify-between items-start px-7">
        <p
          className={`${error || charCount > maxLength ? "text-(--error-red)" : validated ? "text-(--brand-green)" : ""} light`}
        >
          {charCount > maxLength
            ? `Message must include no more than ${maxLength} characters`
            : ""}
        </p>
        <span
          className={`text-xs shrink-0 ml-4 ${charCount > maxLength ? "text-(--error-red)" : charCount > 0 ? "text-(--brand-green)" : "text-slate-500"}`}
        >
          {charCount}/{maxLength}
        </span>
      </div>
