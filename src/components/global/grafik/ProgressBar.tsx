          ...
          {numbers.map((number) => {
            ...
            return (
              <li key={`icon-${number}`} className="flex h-5 justify-center">
                {stepNumber === 1 ? <Card color="foreground" size={20} /> : stepNumber === 2 ? <Wash color="foreground" size={20} /> : <Checkmark color="foreground" size={20} />}
              </li>
            );
          })}
        </ul>
      ...
      <div className="relative">
        ...
        <ul className="relative z-10 flex justify-between gap-10">
          {numbers.map((number) => {
            ...
            const backgroundClass = stepNumber < activeIndex ? "bg-(--brand-green)" : stepNumber === activeIndex ? "bg-foreground" : "bg-background";

            return (
              <li key={number}>
                <div className="flex flex-col items-center gap-2">
                  <div className={`h-10.5 w-10.5 border-2 border-foreground rounded-full ${backgroundClass}`} />
                  {stepNumber === 2 && isFinalStep && isWashProcess ? <span className="text-sm">{formattedDuration}</span> : null}
                </div>
              </li>
            );
          })}
