      ...
      <div className="flex flex-col gap-3">
        {history?.length > 0 ? (
          history.map((wash: any) => (
            <HistorikCard
              key={wash.car_wash_history_pk}
              location={wash.location_name}
              date={new Date(wash.date_of_wash).toLocaleDateString("da-DK")}
              description={wash.car_wash_type}
              price={wash.car_wash_price}
              points={wash.car_wash_price}
              href={`/wash-history/${wash.car_wash_history_pk}`}
            />
          ))
        ) : (
